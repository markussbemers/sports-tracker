            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <form method="POST" action="{{action([App\Http\Controllers\OrganizationController::class, 'store']) }}">
                        @csrf
                        <x-label for="name" :value="__('messages.Name of the organization')" />
                        <x-input id="name" class="block mt-1 w-full" type="text" name="name" required autofocus/>

                        <x-label class="mt-3" for="sports_types_id" :value="__('messages.Sports type of an organization')" />
                        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">

                        <label for="sports_type">{{ __('messages.Choose a sports type') }}:</label>
